...
type ProgressUpdate = {
  operationId: string;
  state: "queued" | "copying" | "verifying" | "formatting" | "completed" | "failed" | "cancelled";
...
function App() {
  ...
  const chooseDestination = useCallback(async () => {
    ...
    if (!("__TAURI_INTERNALS__" in window)) {
      setDestinationPickerLabels((current) => ({
        ...current,
        [selected.id]: "Native folder selection is available in the desktop application. You can type a path in this preview.",
      }));
      return;
    }
    try {
      ...
      setDestinationPickerLabels((current) => ({
        ...current,
        [selected.id]: "The native folder picker could not be opened. You can type a destination path instead.",
      }));
    }
  ...
  const resumeHistoryRun = useCallback(
    async (runId: string) => {
      ...
      progress.onmessage = (update) => {
        ...
        setRecoveryLabels((current) => ({
          ...current,
          [runId]: `${update.state.replace(/_/g, " ")} ${formatTransferBytes(update.transferredBytes)}${update.totalBytes === null ? "" : ` / ${formatTransferBytes(update.totalBytes)}`}`,
        }));
      };
      try {
        const response = await invoke<{
  ...
  const recallDestination = useCallback(async () => {
    if (!selected || !("__TAURI_INTERNALS__" in window)) return;
    try {
      const response = await invoke<{
  ...
  const registerCard = useCallback(async () => {
    ...
    if (!("__TAURI_INTERNALS__" in window)) {
      setRegistrationLabel("Card registration is available in the desktop application.");
      return;
    }
  ...
  const startIngest = useCallback(async () => {
    ...
    progress.onmessage = (update) => {
      ...
      if (update.state === "copying") {
        ...
        setIngestLabels((current) => ({
          ...current,
          [selected.id]: `Copying${detail}${rate}${eta}`,
        }));
        return;
      }
      const detail =
      ...
      setIngestLabels((current) => ({
        ...current,
        [selected.id]: `${update.state.replace(/_/g, " ")}${detail}`,
      }));
    };
  ...
  useEffect(() => {
    ...
    for (const device of liveDevices) {
      ...
      if (!device.ingestable || autoIngestAttempted.current.has(attemptKey)) continue;
      autoIngestAttempted.current.add(attemptKey);
      void invoke<{
        data: CardRegistration | null;
        error: { message: string } | null;
      }>("get_auto_ingest_profile", { sourceMediumKey: device.id }).then(async (response) => {
        const profile = response.data;
        if (!profile?.registered || !profile.autoIngestEnabled || !profile.destinationPath) return;
        ...
        progress.onmessage = (update) => {
          ...
          setIngestLabels((current) => ({
            ...current,
            [device.id]: `Auto-ingest ${update.state.replace(/_/g, " ")} ${formatTransferBytes(update.transferredBytes)}${update.totalBytes === null ? "" : ` / ${formatTransferBytes(update.totalBytes)}`}`,
          }));
        };
        setDestinations((current) => ({ ...current, [device.id]: profile.destinationPath! }));
        setActiveOperations((current) => ({ ...current, [device.id]: { operationId, cancelling: false } }));
        setIngestLabels((current) => ({ ...current, [device.id]: "Registered card detected; starting verified auto-ingest…" }));
        try {
          const response = await invoke<{ data: IngestResult | null; error: { message: string } | null }>("start_verified_ingest", {
            request: { operationId, sourceRoot: device.sourceRoot, destinationRoot: profile.destinationPath, sourceMediumKey: device.id, sourceIdentityConfidence: device.nativeConfidence, sourceGeneration: device.connectionGeneration, maxWorkers: 2, sortMode: profile.sortMode ?? "camera_day", intervalMinutes: profile.sortMode === "camera_interval" ? profile.intervalMinutes : null, autoIngestTriggered: true },
            channel: progress,
          });
          setIngestLabels((current) => ({
            ...current,
          ...
          setIngestLabels((current) => ({ ...current, [device.id]: "The desktop service could not start auto-ingest." }));
        } finally {
          setActiveOperations((current) => { const next = { ...current }; delete next[device.id]; return next; });
        }
      });
    }
  ...
  const openAutoIngestSetup = useCallback(() => {
    if (selected && !destination && !selected.destination.startsWith("Choose a destination")) {
  ...
  if (!selected) {
    return (
      <main className="grid min-h-screen place-content-center gap-4 bg-slate-50 p-8 text-center text-slate-950 dark:bg-slate-950 dark:text-slate-50">
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400">No removable media is mounted. Insert a card and refresh inventory.</p>
        <button
  ...
  return (
    <main className="min-h-screen bg-slate-50 text-slate-950 transition-colors duration-200 dark:bg-slate-950 dark:text-slate-50">
      <header className="flex h-[72px] items-center justify-between border-b border-slate-200 px-5 sm:px-8 dark:border-slate-800">
        <div className="flex items-center gap-3">
          <div className="grid size-8 place-items-center rounded-lg bg-blue-600 text-sm font-bold text-white shadow-sm" aria-hidden="true">
            M
          </div>
          <div>
            <p className="mb-0.5 text-[10px] font-bold tracking-[0.14em] text-blue-600 dark:text-blue-400">LOCAL MEDIA INGEST</p>
        ...
        <div className="flex items-center gap-3">
          ...
          <button className="hidden rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-700 shadow-sm transition hover:border-blue-300 hover:text-blue-700 sm:block dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200" type="button">
            Settings
          </button>
        </div>
      </header>
      <section className="grid min-h-[calc(100vh-114px)] grid-cols-1 lg:grid-cols-[19rem_minmax(0,1fr)]" aria-label="Media ingest workspace">
        <aside className="border-b border-slate-200 bg-white px-4 py-6 dark:border-slate-800 dark:bg-slate-900/40 lg:border-r lg:border-b-0" aria-labelledby="connected-media-title">
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="mb-1 text-[10px] font-bold tracking-[0.14em] text-slate-400 dark:text-slate-500">LIVE INVENTORY</p>
              <h2 id="connected-media-title" className="text-sm font-semibold tracking-tight">Connected media</h2>
            </div>
            <span className="grid size-6 place-items-center rounded-md bg-blue-50 text-xs font-semibold text-blue-700 dark:bg-blue-500/10 dark:text-blue-300">{liveDevices.length}</span>
          </div>
            ...
            {liveDevices.map((device) => {
              ...
              return (
                <button
                  ...
                  type="button"
                >
                  <span className="grid size-8 place-items-center rounded-lg bg-blue-100 text-blue-700 dark:bg-blue-500/15 dark:text-blue-300" aria-hidden="true">
                    ▣
                  </span>
                  <span className="grid min-w-0 flex-1 gap-1">
                    ...
                    <span className="text-slate-500 dark:text-slate-400">{device.slot}</span>
                  </span>
                </button>
              );
            })}
          ...
          <button
            ...
            type="button"
          >
            <span className="mr-2 text-blue-600 dark:text-blue-400" aria-hidden="true">＋</span>No other removable media detected
          </button>
        </aside>
        <section className="mx-auto w-full max-w-7xl px-5 py-8 sm:px-8 sm:py-12" aria-labelledby="device-title">
          <div className="flex flex-col justify-between gap-6 border-b border-slate-200 pb-8 md:flex-row dark:border-slate-800">
            <div>
              <div className="mb-3 flex items-center gap-3">
                ...
                <span className="text-xs text-slate-500 dark:text-slate-400">{selected.updated}</span>
              </div>
              <h2 id="device-title" className="text-3xl font-semibold tracking-tight sm:text-4xl">{selected.name}</h2>
          ...
          <div className="my-8 grid divide-y divide-slate-200 border-y border-slate-200 sm:grid-cols-3 sm:divide-x sm:divide-y-0 dark:divide-slate-800 dark:border-slate-800">
            <article className="py-4 sm:px-5 sm:first:pl-0">
              ...
              <span className="mt-2 block text-xs leading-5 text-slate-500 dark:text-slate-400">Read-only while copying; marker only after verification</span>
            </article>
            <article className="py-4 sm:px-5">
              ...
              <span className="mt-2 block text-xs leading-5 text-slate-500 dark:text-slate-400">Bounded device-aware workers</span>
            </article>
            <article className="py-4 sm:px-5">
              ...
              <span className="mt-2 block text-xs leading-5 text-slate-500 dark:text-slate-400">Fresh destination readback required</span>
            </article>
          </div>
          <div className="grid gap-8 lg:grid-cols-[minmax(0,1.2fr)_minmax(17rem,0.8fr)]">
            <article>
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="mb-1 text-[10px] font-bold tracking-[0.14em] text-slate-400 dark:text-slate-500">INGEST PLAN</p>
                  <h3 className="text-base font-semibold tracking-tight">Destination & organization</h3>
                </div>
                <button className="text-xs font-semibold text-blue-600 hover:text-blue-700 dark:text-blue-400" type="button">
                  Change
                </button>
              </div>
              ...
              {selectedDestinationPickerLabel ? <p className="mt-2 text-xs leading-5 text-slate-500 dark:text-slate-400" aria-live="polite">{selectedDestinationPickerLabel}</p> : null}
              <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-2">
                ...
                <span className="text-xs text-slate-500 dark:text-slate-400" aria-live="polite">{selectedDestinationMemoryLabel}</span>
              </div>
              <div className="mt-5 flex flex-wrap items-center gap-3 border-t border-slate-200 pt-5 dark:border-slate-800">
                ...
                <span className="text-xs text-slate-500 dark:text-slate-400" aria-live="polite">{registrationLabel}</span>
              </div>
              ...
              <div className="flex items-center justify-between gap-4 border-t border-slate-200 py-3 text-xs dark:border-slate-800">
                <span className="text-slate-500 dark:text-slate-400">Camera identity</span>
              ...
              <div className="mt-5 flex items-center gap-3">
                ...
                <span className="text-xs text-slate-500 dark:text-slate-400" aria-live="polite">{selectedInventoryLabel}</span>
              </div>
              <div className="mt-3 flex items-center gap-3">
                ...
                <span className="text-xs text-slate-500 dark:text-slate-400" aria-live="polite">{selectedPlanPreviewLabel}</span>
              </div>
              {selectedPlanPreview ? (
                <div className="mt-3 grid gap-1 rounded-lg border border-blue-100 bg-blue-50 p-3 text-xs text-slate-600 dark:border-blue-500/20 dark:bg-blue-500/10 dark:text-slate-300" aria-live="polite">
                  ...
                  {selectedPlanPreview.sampleDestinationPaths.map((path) => (
                    <code className="overflow-hidden text-ellipsis whitespace-nowrap text-blue-700 dark:text-blue-300" key={path}>{path}</code>
                  ))}
                </div>
              ) : null}
              <button
              ...
              <p className="mt-3 min-h-5 text-xs leading-5 text-slate-500 dark:text-slate-400" aria-live="polite">
                {selectedIngestLabel}
              </p>
            </article>
            <article className="border-t border-slate-200 pt-8 dark:border-slate-800 lg:border-t-0 lg:border-l lg:pt-0 lg:pl-8">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="mb-1 text-[10px] font-bold tracking-[0.14em] text-slate-400 dark:text-slate-500">HARDWARE EVIDENCE</p>
                  <h3 className="text-base font-semibold tracking-tight">Identity & reader</h3>
                </div>
                <button className="text-xs font-semibold text-blue-600 hover:text-blue-700 dark:text-blue-400" type="button">
                  View details
                </button>
              ...
              <dl className="mt-5">
                <div className="border-t border-slate-200 py-3 dark:border-slate-800">
                  <dt className="mb-1 text-[11px] text-slate-500 dark:text-slate-400">Identity evidence</dt>
                  <dd className="m-0 font-mono text-xs leading-5 text-slate-700 dark:text-slate-200">{selected.identity}</dd>
                </div>
                <div className="border-t border-slate-200 py-3 dark:border-slate-800">
                  <dt className="mb-1 text-[11px] text-slate-500 dark:text-slate-400">Reader</dt>
                  <dd className="m-0 font-mono text-xs leading-5 text-slate-700 dark:text-slate-200">{selected.reader}</dd>
                </div>
                <div className="border-t border-slate-200 py-3 dark:border-slate-800">
                  <dt className="mb-1 text-[11px] text-slate-500 dark:text-slate-400">Physical location</dt>
                  <dd className="m-0 font-mono text-xs leading-5 text-slate-700 dark:text-slate-200">{selected.slot}</dd>
                </div>
                <div className="border-t border-slate-200 py-3 dark:border-slate-800">
                  <dt className="mb-1 text-[11px] text-slate-500 dark:text-slate-400">Volume reference</dt>
                  <dd className="m-0 font-mono text-xs leading-5 text-slate-700 dark:text-slate-200">Observed only · never used as identity</dd>
                </div>
              </dl>
              {selected.readerFingerprint && selected.logicalUnit !== null ? (
                <div className="mt-4 grid gap-3 border-t border-slate-200 pt-4 dark:border-slate-800">
                  <p className="m-0 text-xs text-slate-500 dark:text-slate-400">Controlled slot calibration</p>
                  ...
                  <span className="text-xs text-slate-500 dark:text-slate-400" aria-live="polite">{calibrationLabel}</span>
                </div>
              ) : null}
            </article>
          </div>
          <article className="mt-8 flex flex-wrap items-center gap-4 rounded-xl border border-blue-100 bg-blue-50 p-5 dark:border-blue-500/20 dark:bg-blue-500/10">
            <div className="grid size-9 place-items-center rounded-full bg-blue-600 text-sm font-bold text-white" aria-hidden="true">
              ✓
            </div>
            <div>
              <p className="mb-1 text-[10px] font-bold tracking-[0.14em] text-blue-700 dark:text-blue-300">VERIFICATION GATE</p>
            ...
            <button className="ml-auto text-xs font-semibold text-blue-700 hover:text-blue-800 dark:text-blue-300" type="button">
              Open receipt
            </button>
          </article>
          <article className="mt-8 border-t border-slate-200 pt-8 dark:border-slate-800" aria-labelledby="history-title">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="mb-1 text-[10px] font-bold tracking-[0.14em] text-slate-400 dark:text-slate-500">LOCAL HISTORY</p>
                <h3 id="history-title" className="text-base font-semibold tracking-tight">Recent ingests</h3>
              </div>
              <button
                className="text-xs font-semibold text-blue-600 hover:text-blue-700 dark:text-blue-400"
            ...
            {history.length ? (
              <ul className="mt-4 grid list-none gap-0 p-0">
                {history.map((run) => (
                  <li className="flex items-center justify-between gap-4 border-t border-slate-200 py-3 text-xs dark:border-slate-800" key={run.runId}>
                    ...
                    {run.state === "recovery_required" ? (
                      <span className="grid justify-items-end gap-1 text-right">
                        ...
                        {recoveryLabels[run.runId] ? (
                          <small className="text-[11px] text-slate-500 dark:text-slate-400">{recoveryLabels[run.runId]}</small>
                        ) : null}
                      </span>
                    ) : null}
                  </li>
              ...
              <p className="mt-4 text-xs text-slate-500 dark:text-slate-400">No completed native ingest history yet.</p>
            )}
          </article>
        </section>
      </section>
      {isAutoIngestSetupOpen ? (
        <div
          ...
          tabIndex={-1}
        >
          <section className="w-full max-w-lg rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl dark:border-slate-700 dark:bg-slate-900" onClick={(event) => event.stopPropagation()}>
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="mb-1 text-[10px] font-bold tracking-[0.14em] text-blue-600 dark:text-blue-400">REGISTERED CARD</p>
                <h2 id="auto-ingest-title" className="text-xl font-semibold tracking-tight">Set up auto-ingest</h2>
                <p className="mt-2 text-sm leading-6 text-slate-500 dark:text-slate-400">
                  Configure {selected.name} to begin one verified ingest on a future matching mount.
                </p>
              </div>
              <button
            ...
            <div className="mt-5 grid gap-3 rounded-xl bg-slate-50 p-4 dark:bg-slate-800/70">
              <label className="flex cursor-pointer items-start gap-3 text-sm text-slate-700 dark:text-slate-200">
                <input
                  ...
                  type="checkbox"
                />
                <span><strong className="block font-semibold">Ingest automatically on mount</strong><span className="mt-1 block text-xs leading-5 text-slate-500 dark:text-slate-400">Requires a fresh exact marker match and the destination above to remain available.</span></span>
              </label>
              <label className="flex cursor-pointer items-start gap-3 text-sm text-slate-700 opacity-100 dark:text-slate-200">
                <input
                  ...
                  type="checkbox"
                />
                <span className={!autoIngestEnabled ? "opacity-45" : ""}><strong className="block font-semibold">Format after a verified auto-ingest</strong><span className="mt-1 block text-xs leading-5 text-slate-500 dark:text-slate-400">Only runs when the native safety gates and platform format provider allow it.</span></span>
              </label>
            </div>
            <div className="mt-5 flex items-start gap-2 rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs leading-5 text-amber-900 dark:border-amber-500/25 dark:bg-amber-500/10 dark:text-amber-200">
              ...
              <p className="m-0">The card marker is mutable continuity evidence, not immutable identity. It cannot authorize formatting by itself.</p>
            </div>
            {registrationLabel ? <p className="mt-4 text-xs text-slate-500 dark:text-slate-400" aria-live="polite">{registrationLabel}</p> : null}
